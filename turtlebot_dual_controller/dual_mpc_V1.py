...
This node controls two robots that are carrying a long object. The aim is to drive
...
Key functionalities:
  - Subscribes to /robot1/pose and /robot2/pose for robot pose updates.
  ...
  - Sets up a differential-drive model and MPC (using do_mpc) with constraints:
      ...
      * Obstacle avoidance constraints keep both robots and the object center outside
        safety zones around obstacles.
...
class DualMPCController(Node):
    """
    DualMPCController for two robots carrying a long item.
    Goal pose and obstacles are updated via ROS2 topics.
    """

    class SimpleKF:
        """
        A simple Kalman filter for 3D state [x, y, theta].
    ...
    def __init__(self):
        ...
        # ----------------------------
        self.goal_pose = np.array([0.6, 2.2, np.pi/2])  # [x, y, theta]
        self.obstacles = [(1.0,1.5,0.4)]  # List of tuples: (obs_x, obs_y, obs_radius)
        ...
        # ----------------------------
        self.declare_parameter("max_velocity", 0.015)
        self.declare_parameter("max_angular_velocity", 0.015)
        self.declare_parameter("min_distance", 0.35)
        self.declare_parameter("max_distance", 0.7)
    ...
    # -------------------------------------------------------------------------
    def robot1_pose_callback(self, msg: Pose2D):
        """
        Callback for /robot1/pose. Updates raw and filtered state for robot1.
        """
        measurement = np.array([msg.x, msg.y, msg.theta])
        self.robot1_state_raw = measurement
        self.robot1_state_filt = self.kf_robot1.update(measurement)
        self.pose1_received = True
        self.get_logger().info(f"Robot1 raw: {measurement} | Filtered: {self.robot1_state_filt}")

    def robot2_pose_callback(self, msg: Pose2D):
        """
        Callback for /robot2/pose. Updates raw and filtered state for robot2.
        """
        measurement = np.array([msg.x, msg.y, msg.theta])
        self.robot2_state_raw = measurement
        self.robot2_state_filt = self.kf_robot2.update(measurement)
        self.pose2_received = True
        self.get_logger().info(f"Robot2 raw: {measurement} | Filtered: {self.robot2_state_filt}")
    ...
    def setup_model(self):
        ...
        # Differential-drive kinematics for Robot2.
    ...
    def setup_model_with_inertia(self):
        """
        Sets up a dynamic model that includes kinetic effects for each robot.
        Each robot's state vector is:
        [x, y, theta, v, omega]
        and the control inputs are force (F) and torque (tau).
        
        The dynamics include frictional (damping) terms and use inertia parameters.
        
        Equations for each robot:
        - x_dot = v * cos(theta)
        - y_dot = v * sin(theta)
        - theta_dot = omega
        - v_dot = (F/mass) - (friction_linear * v)
        - omega_dot = (tau/inertia) - (friction_angular * omega)
        """
        import do_mpc
        import casadi as ca
        ...
        # ----------------------------
        # Define state variables for Robot 1:
        # ----------------------------
        model.set_variable(var_type='_x', var_name='x1')
        model.set_variable(var_type='_x', var_name='y1')
        model.set_variable(var_type='_x', var_name='theta1')
        model.set_variable(var_type='_x', var_name='v1')      # Linear velocity
        model.set_variable(var_type='_x', var_name='omega1')    # Angular velocity

        # ----------------------------
        # Define state variables for Robot 2:
        # ----------------------------
        model.set_variable(var_type='_x', var_name='x2')
        model.set_variable(var_type='_x', var_name='y2')
        model.set_variable(var_type='_x', var_name='theta2')
        model.set_variable(var_type='_x', var_name='v2')
        model.set_variable(var_type='_x', var_name='omega2')

        # ----------------------------
        # Define control inputs:
        # ----------------------------
        # For Robot 1:
        model.set_variable(var_type='_u', var_name='F1')      # Force command (linear)
        model.set_variable(var_type='_u', var_name='tau1')      # Torque command (angular)
        # For Robot 2:
        model.set_variable(var_type='_u', var_name='F2')
        model.set_variable(var_type='_u', var_name='tau2')

        # ----------------------------
        # Define constant parameters (could also be declared as ROS2 parameters)
        # ----------------------------
        mass = 1.0               # Mass of the robot [kg]
        inertia = 0.1            # Moment of inertia [kg·m^2]
        friction_linear = 0.01    # Linear friction coefficient [1/s]
        friction_angular = 0.01   # Angular friction coefficient [1/s]

        # ----------------------------
        # Define the dynamics for Robot 1:
        # ----------------------------
        # Position update
        model.set_rhs('x1', model.x['v1'] * ca.cos(model.x['theta1']))
        model.set_rhs('y1', model.x['v1'] * ca.sin(model.x['theta1']))
        # Orientation update
        model.set_rhs('theta1', model.x['omega1'])
        # Linear acceleration: control force divided by mass minus friction effect.
        model.set_rhs('v1', model.u['F1'] / mass - friction_linear * model.x['v1'])
        # Angular acceleration: control torque divided by inertia minus friction effect.
        model.set_rhs('omega1', model.u['tau1'] / inertia - friction_angular * model.x['omega1'])

        # ----------------------------
        # Define the dynamics for Robot 2:
        # ----------------------------
        model.set_rhs('x2', model.x['v2'] * ca.cos(model.x['theta2']))
        model.set_rhs('y2', model.x['v2'] * ca.sin(model.x['theta2']))
        model.set_rhs('theta2', model.x['omega2'])
        model.set_rhs('v2', model.u['F2'] / mass - friction_linear * model.x['v2'])
        model.set_rhs('omega2', model.u['tau2'] / inertia - friction_angular * model.x['omega2'])

        # Finalize model setup.
        model.setup()
        self.get_logger().info("Dynamic model with kinetics (including friction, inertia, and omega') setup complete.")
        return model
    ...
    #     - An obstacle avoidance constraint (for one obstacle) that keeps both robots and the object center
    #         outside the obstacle plus a safety margin.
    #     """
    ...
    #     # Compute the center of the object carried by the robots.
    ...
    #     Q_center = ca.diag(ca.vertcat(25, 25, 0.1))
    ...
    #     # The squared distance should be between (min_distance)^2 and (max_distance)^2.
    ...
    #         # Obstacle avoidance constraints.
    #         # 
    #         #   self.obstacle_pos = (obstacle_x, obstacle_y)
    #         #   self.obstacle_radius = radius of obstacle (if any physical extent)
    #         #   self.margin = additional safety margin
    #         obstacle_x, obstacle_y = self.obstacles[0][:2]
    #         margin = 0.1
    #         # Compute squared distances from each robot to the obstacle.
    ...
    #         # Compute squared distance from the object's center to the obstacle.
    ...
    #         # To ensure the squared distance is >= safety_dist_sq,
    #         # we enforce: -distance <= -safety_dist_sq.
    ...
    def define_obstacle_constraints(self, mpc, model):
        ...
        for i, obstacle in enumerate(self.obstacles):
            ...
            self.get_logger().info(f"Obstacle avoidance constraints added for obstacle {i+1}.")             
    ...
    def control_loop(self):
        ...
        - If a new goal message was received, update the MPC configuration.
        - Wait until both robot poses have been received.
        - Concatenate the filtered states to form the MPC state vector.
        - Compute the MPC step and publish the control commands.
        """
        ...
        self.get_logger().info(
            f"Control Commands: Robot1: v={v1_cmd:.2f}, omega={omega1_cmd:.2f} | "
            f"Robot2: v={v2_cmd:.2f}, omega={omega2_cmd:.2f}"
        )

        # Publish Twist commands (invert sign if needed to match your robot's frame).
        twist1 = Twist()
